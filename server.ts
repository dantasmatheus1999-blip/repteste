...
async function startServer() {
  ...
  // Lista os arquivos .png da pasta mostro/ no Firebase Storage como Biblioteca Padrão
  app.get("/api/storage/mostro-monsters", async (req, res) => {
    const bucket = serverFirebaseConfig?.storageBucket || "gen-lang-client-0150741197.firebasestorage.app";
    
    // Extrai o token do cabeçalho Authorization ou query string
    let rawToken = "";
    ...
    // 1. Tentar via API REST do Firebase Storage (executada pelo servidor sem restrições de CORS)
    try {
      const endpointsToTry = [
        `https://firebasestorage.googleapis.com/v0/b/${encodeURIComponent(bucket)}/o?prefix=mostro%2F`,
        `https://firebasestorage.googleapis.com/v0/b/${encodeURIComponent(bucket)}/o?prefix=mostro`
      ];

      const authHeadersToTry: Record<string, string>[] = [];
      if (rawToken) {
        authHeadersToTry.push({ Authorization: `Bearer ${rawToken}` });
        authHeadersToTry.push({ Authorization: `Firebase ${rawToken}` });
      }
      // Sempre tenta também sem token (para o caso de leitura pública permitida)
      authHeadersToTry.push({});

      for (const endpoint of endpointsToTry) {
        for (const headers of authHeadersToTry) {
          try {
            const restRes = await fetch(endpoint, { headers });
            if (restRes.ok) {
              const data: any = await restRes.json();
              if (Array.isArray(data.items) && data.items.length > 0) {
                const pngItems = data.items.filter((item: any) => 
                  typeof item.name === "string" && item.name.toLowerCase().endsWith(".png")
                );

                if (pngItems.length > 0) {
                  const monsters = pngItems.map((item: any) => {
                    const fileName = item.name.split("/").pop() || item.name;
                    const name = formatName(fileName);
                    const tokenParam = item.downloadTokens 
                      ? `&token=${item.downloadTokens.split(",")[0]}` 
                      : "";
                    const url = `https://firebasestorage.googleapis.com/v0/b/${encodeURIComponent(bucket)}/o/${encodeURIComponent(item.name)}?alt=media${tokenParam}`;
                  ...
                  monsters.sort((a: any, b: any) => a.name.localeCompare(b.name, "pt-BR"));
                  console.log(`[Server] Sucesso na listagem REST da pasta mostro/: ${monsters.length} monstros carregados.`);
      ...
      console.warn("[Server] Aviso na busca REST da pasta mostro/:", restErr?.message);
    }

    // 2. Tentar via SDK do Firebase Storage do servidor
    try {
      if (serverStorage) {
        const folderRef = storageRef(serverStorage, "mostro");
        ...
        for (const prefix of listResult.prefixes) {
          try {
            ...
            allItems.push(...subRes.items);
          } catch (subErr) {
            // ignora erro em subpasta
          }
        }

        const pngItems = allItems.filter((item: any) => item.name.toLowerCase().endsWith(".png"));
        if (pngItems.length > 0) {
          const monsters = await Promise.all(
            pngItems.map(async (item: any) => {
              try {
          ...
          const valid = monsters.filter(Boolean);
          valid.sort((a: any, b: any) => a.name.localeCompare(b.name, "pt-BR"));
          console.log(`[Server] Sucesso na listagem SDK da pasta mostro/: ${valid.length} monstros carregados.`);
          return res.json({ monsters: valid });
        }
      }
    } catch (err: any) {
      console.warn("[Server] Aviso ao ler pasta mostro/ via SDK:", err.message);
    }
