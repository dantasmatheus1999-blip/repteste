...
interface DeityCardProps {
  deity: T20DeityDetail;
  onClick: () => void;
}

export const DeityCard: React.FC<DeityCardProps> = ({ deity, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex gap-5">
        <div className="w-16 h-16 shrink-0 rounded-full bg-black/40 border border-gold/20 flex items-center justify-center text-gold group-hover:scale-110 group-hover:border-gold/50 transition-all duration-500 shadow-inner">
        ...
        <div className="flex-1 space-y-2">
          ...
          <div className="pt-2 flex flex-wrap gap-2">
            <span className="text-[9px] px-2 py-0.5 bg-gold/5 border border-gold/10 text-gold/60 rounded-sm font-bold uppercase tracking-wider">
              {deity.grantedPowerIds?.length || 0} Poderes
            </span>
          </div>
        </div>
      </div>
