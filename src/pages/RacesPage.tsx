...
import { RaceCard } from '../components/codex/RaceCard';
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 relative pb-6">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-cinzel text-gold-gradient drop-shadow-lg tracking-tighter">Raças</h2>
          </div>
          <p className="text-gold/40 text-xs sm:text-sm font-medium tracking-widest italic font-cinzel">"As linhagens que moldam o destino de Arton."</p>
        </div>
      ...
      <div className="relative group">
        ...
        <input 
          type="text"
          placeholder="Buscar raça pelo nome ou essência..."
          value={searchTerm}
      ...
      {filteredRaces && filteredRaces.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRaces.map(r => (
            <RaceCard 
              key={r.id} 
              race={r} 
              onClick={() => navigate(`/codex/racas/${r.slug}`)} 
            />
          ))}
        </div>
      ) : (
        <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
          ...
          <div className="space-y-1">
            <h3 className="text-2xl font-cinzel text-gold/60">Nenhuma linhagem encontrada</h3>
            <p className="text-gold/30 italic">"Talvez este povo ainda não tenha sido catalogado..."</p>
          </div>
        </div>
      )}
    </div>
